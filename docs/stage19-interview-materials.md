...
本文件是历史详细面试题扩展材料；当前项目级评分以 `docs/08-final-acceptance.md` 为准，为 **83/100**。本地工程基线有条件通过，生产发布不通过。该分数不是 QPS、可用性或安全认证；真实 Chromium direct/proxy 最终各 19/19，但生产 Ingress、跨实例广播、HA 和密钥轮换仍有明确边界。
...
| “项目实现了生产级 Kubernetes 高可用” | 当前不能使用；仅在 Kind 单节点完成应用层部署、Pod 恢复和滚动重启验证 |
| “系统支持某个 QPS/用户量” | 当前不能使用；已有本机基线，但不能外推到目标生产环境 |
...
44. **Kubernetes 阶段验证到什么程度？**  已完成 Namespace、ConfigMap、Secret、Deployment、Service、探针和资源配置的 Kustomize 渲染；Kind `dev` 中后端 2/2、前端 1/1 运行，端口转发健康检查返回 HTTP 200，后端 Pod 删除恢复和前后端滚动重启均通过。
45. **如何验证 Pod 自动恢复？**  在已认证的本地集群中删除一个后端 Pod，观察 Deployment 重新创建 Pod、readiness 通过后加入 Service Endpoints，再执行 rollout status；本项目已完成该 Kind 实机验证。
46. **项目有没有真实 QPS 或延迟指标？**  已有一组 Windows 11 本机基线：固定 1000 个任务、20 并发、预热 10 秒、采样 60 秒，六个场景均 0 错误；这只能作为可复核的本地测试记录，不能直接写成生产容量指标。
...
49. **项目哪些部分不能说成生产能力？**  默认开发凭据、单实例中间件、未完成 TLS/集中密钥管理、Kind 单节点验证外的 Kubernetes 高可用、目标环境容量和未执行的在线漏洞扫描都不能说成生产能力；登录限流已在本地 Compose 烟测验证，但不等同于生产安全基线。
...
- Kind `dev` 中后端 Deployment 运行副本数为 2，前端运行副本数为 1；已通过本地探针、恢复和滚动重启验证；
- 阶段 15 本机基线固定为 20 并发、预热 10 秒、采样 60 秒，六个 HTTP 场景错误率为 0；
...
- 生产级 Kubernetes 高可用和多节点故障恢复；
- 任何未经目标环境复测的 QPS、延迟、用户量和容量承诺；
...
| 容易被追问的表述 | 更准确的说法 |
| --- | --- |
| “实现了 Kubernetes 高可用” | “编写了应用层 Kubernetes 清单，配置了双副本和滚动更新，并在 Kind 单节点完成 Pod 恢复验证；生产 HA 尚未验证” |
| “系统支持高并发” | “实现了乐观锁、批量关联查询和有限消息重试，并完成一组本机基线；目标环境容量需要复测” |
...
| “做了性能优化并提升了 30%” | “提供了数据准备、压测和 EXPLAIN 工具，并保存本机基线；没有未经对照实验的提升比例” |
