...
5. 阶段 5 真实完成 Redis、RabbitMQ、MinIO 的短暂停止/恢复，其中 Redis 会话恢复、MinIO 上传失败后的 FAILED 元数据状态和核心 MySQL 计数不变均有证据；RabbitMQ 真实非法消息到 retry/DLQ/replay、MySQL 本阶段保卷重启仍未完成。阶段 10 已新增真实浏览器 WebSocket 重连和离线通知补拉证据，但阶段 11 复验区分出后端直连和 Nginx `/ws` 代理两条链路。阶段 1 已完成生产弱 Secret 拒绝、管理面收紧、安全 Header 和 Secret 模板治理；阶段 2 已增加覆盖率门禁、两层 CI 和依赖扫描可见性；阶段 11 npm audit 已清零并使 OWASP/npm 失败阻断 CI，但 OWASP 仍有 19 个依赖条目、79 条 high 和 25 条 critical，生产 TLS/WSS、外部 Secret 轮换和多节点中间件高可用仍未完成。
...
阶段 0 调优冻结记录见[优化基线账本](optimization-baseline-2026-08-09.md)。阶段 1/2 未改变业务 API 契约；阶段 2 增加 JaCoCo、两层 GitHub Actions、npm audit/OWASP 扫描入口和统一门禁文档。阶段 11 复验中 npm 官方 audit 已为 moderate/high/critical 全 0；OWASP 使用本机缓存生成报告但 hosted suppressions/在线数据源不完整且存在 high/critical 命中，Maven 仍以 exit 1 结束，未被计为“扫描通过”。
...
| 11（调优阶段） | 依赖漏洞治理、CI 质量门禁真实闭环 | React Router 升级后 npm audit 0/0/0；OWASP 报告生成但 exit 1，CI 最终 gate 已 fail-closed；远程 workflow 尚未验证 |
...
- Kind 本地 edge 已验证 HTTPS 首页、`/api/health` 和 WSS 101 握手；生产部署仍必须使用受控 Ingress/反向代理、真实证书链、可信代理边界和安全 Header 策略。阶段 11 复验表明后端直连 Chromium 可收到 STOMP MESSAGE，但当前前端 Nginx `/ws` 代理路径未闭环，不能将直连证据包装成代理或生产证据。
- MySQL、Redis、RabbitMQ、MinIO 在本地拓扑中均为单实例，Kubernetes 只托管前后端应用层，不构成生产高可用。
- npm 官方 audit 当前 moderate/high/critical 均为 0，但这不等于 Maven/npm/OWASP 依赖零漏洞；OWASP Dependency-Check 报告仍含 79 high、25 critical，并因数据源/hosted suppression 边界以失败结束。阶段 15 已有本机性能基线，阶段 8 仅完成 acceptance 小规模数据准备，最终同参数复测尚未完成。
