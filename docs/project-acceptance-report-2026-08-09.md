...
> **阶段 1 后项目级评分建议：80/100。结论：本地工程基线有条件通过，可用于学习、演示和面试；生产交付仍不通过。**
...
5. 阶段 1 已完成生产弱 Secret 拒绝、管理面收紧、安全 Header 和 Secret 模板治理；生产级 Token 存储、TLS/WSS、外部 Secret 轮换、依赖漏洞扫描、多节点中间件高可用和完整浏览器 E2E 仍未完成。

阶段 0 调优冻结记录见[优化基线账本](optimization-baseline-2026-08-09.md)。本阶段没有业务代码改动，没有调整 79/100 评分；`docker compose config --quiet` 因 Windows 审批通道传输失败未获得新的可靠退出结果，现有 Compose 运行态仍为六服务 healthy。
...
| 安全与数据保护 | 15% | 73 | 10.95 | 已验证后端鉴权、会话撤销、登录窗口限流、权限/数据范围、附件签名校验、prod 弱 Secret 拒绝、安全 Header 和管理面收紧；Token localStorage、明文本地链路、外部 Secret 轮换和生产代理隔离仍扣分 |
| 测试、构建与回归 | 15% | 85 | 12.75 | Maven 66/0/1、阶段14 Testcontainers、前端构建、Compose 模板解析和 Kind 清单/运行验证均有证据；可选测试默认跳过，完整浏览器 E2E 不足 |
| 评估可信度与可观测性 | 15% | 78 | 11.70 | 已有性能基线、运行时指标、EXPLAIN、Actuator 和报告文件；数据仅来自单机，在线依赖扫描和目标环境复测未完成 |
| 运行集成与恢复就绪度 | 15% | 76 | 11.40 | 六服务 Compose 健康、保卷重启、Kind 前后端探针/Pod 恢复/滚动重启通过；中间件仍是单实例，无 Ingress/TLS/HPA/多节点 HA |
| 工程治理 | 5% | 58 | 2.90 | 有阶段文档、脚本、缓存路径和变更记录；尚未形成完整 CI、覆盖率门槛、在线依赖扫描和统一质量门禁 |
| 文档与交付可信度 | 5% | 92 | 4.60 | README、CHANGELOG、阶段文档、中文面试材料和本报告已同步，明确区分本地证据与生产边界 |
| **合计** | **100%** |  | **80.10 → 80** | **本地工程基线有条件通过；生产就绪不通过** |
...
| 命令 | 结果 |
| --- | --- |
| `.\mvnw.cmd test` | 通过：66 项执行，失败 0，跳过 1 |
...
| 本地工程基线 | **有条件通过，80/100** | 主要测试、构建、部署和性能证据已留档；安全基线有所收敛，但仍有 P0/P1 项 |
...
- 当前默认回归测试为 64 项执行、0 失败、1 项可选 Testcontainers 测试跳过；显式阶段 14 Testcontainers 测试另行通过 1 项。Compose 六服务健康、Kind 应用层实机验证和阶段 15 本地性能基线均有留档。
