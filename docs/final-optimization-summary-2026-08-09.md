# TaskFlow Platform 阶段 0～8 最终复验摘要（2026-08-09）
...
已新增隔离 acceptance profile、专属六服务 Compose、环境变量凭据注入和 BCrypt 管理员初始化。`scripts/acceptance-check.ps1` 真实通过 health、登录、`/api/auth/me`、任务列表、登出及旧会话 401；性能工具使用同一组 acceptance 变量完成少量数据准备。默认 Maven 为 69/0/1，显式 Testcontainers verify 为 69/0/0。Playwright 真实执行为 6/9 通过、3/9 失败（重复提交按钮断言、STOMP 通知和断线补拉），完整性能复测仍未执行，因此本补充不调整 80/100 评分。
...
| 测试、构建与回归 | 15% | 84 | 12.60 | Maven 69/0/1 默认回归通过；显式 Testcontainers 69/0/0 通过；JaCoCo 行覆盖率 47.70%；前端 install/build 通过；acceptance E2E 为 6 通过、3 失败 |
...
4. 建立 Maven/JaCoCo、Testcontainers、npm build、Kustomize、Compose health 和 Playwright 的分层质量门禁；阶段 8 后默认回归 69/0/1，集成 verify 69/0/0，acceptance smoke 可重复执行。
