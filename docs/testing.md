...
阶段 5 增加 `scripts/fault-injection.ps1` 和本地故障证据。脚本默认只做计划检查，必须显式传入 `-Execute` 才停止/启动精确的 TaskFlow 容器；它不会执行 `compose down`、删除卷或清理数据库。当前真实通过 Redis/MinIO 短故障恢复和 RabbitMQ 容器恢复；真实消息 retry/DLQ/replay、MySQL 本阶段保卷重启和浏览器 WebSocket 重连仍未完成，详见 `docs/fault-injection-2026-08-09.md`。
